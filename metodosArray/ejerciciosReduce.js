...
const filterNat = (usersArray, nat) =>{
    let result = usersArray.filter((user)=>user.nat === nat)
    return result
}

console.log(filterNat(users, "AU"))
...
const filterGender = (usersArray, gender) =>{
    let result = usersArray.filter((user)=>user.gender === gender)
    return result
}

console.log(filterGender(users, "female"))


const listUsers = (usersArray) =>{
    let result = usersArray.reduce((accum,current)=>{
        return [...accum, `Full Name: ${current.name.first} ${current.name.last} / Address: ${current.location.street.name} #${current.location.street.number}, ${current.location.city}, ${current.location.state}, ${current.location.country}, Postal Code ${current.location.postcode}`]
    }, [])
    return result
}
